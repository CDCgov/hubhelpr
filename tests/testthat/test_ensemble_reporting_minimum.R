test_that("reference dates before the minimum was introduced require no models", {
  # the hub reported its ensemble without a reporting
  # rule until 2026-04-04, so regenerating those weeks
  # under today's minimum designated number of models
  # would drop rows that were originally published
...
test_that("the record is ordered, so the latest applicable row is what is used", {
  expect_identical(
