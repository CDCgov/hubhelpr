...
> **Note:** This directory is a lightweight, downsampled test snapshot bundled with the `hubhelpr` package for automated testing. For full, up-to-date operational hub data, see the live repository at <https://github.com/CDCgov/covid19-forecast-hub>. The covidhub repository content is Apache-2.0 licensed and reused here on that basis (<https://github.com/CDCgov/covid19-forecast-hub/blob/main/LICENSE>). All real model data included in this downsampled snapshot are licensed under CC-BY-4.0 and attributed to the original model submitters listed in each corresponding `model-metadata` file (license: <https://creativecommons.org/licenses/by/4.0/>).
