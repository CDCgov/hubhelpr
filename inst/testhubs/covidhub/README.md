...
> **Note: test snapshot:** This is a downsampled and modified test snapshot of the COVID-19 Foreacst Hub bundled with the `hubhelpr` package for testing. For the complete Hub, see <https://github.com/CDCgov/covid19-forecast-hub>. COVIDHub repository content is Apache-2.0 licensed and reused here on that basis (<https://github.com/CDCgov/covid19-forecast-hub/blob/main/LICENSE>). All real model output included in this snapshot is CC-BY-4.0-licensed and attributed to the original model submitters listed in each corresponding `model-metadata` file (license: <https://creativecommons.org/licenses/by/4.0/>). The `designated_model` and `designated_targets` fields of some `model-metadata` files have been modified from their original versions for testing purposes. Some submission files have been converted from `.csv` to parquet.
